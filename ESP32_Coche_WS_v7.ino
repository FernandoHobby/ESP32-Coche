...
void setup() {
  ...
  digitalWrite(LVOL, LOW);
  digitalWrite(LCON, LOW);
  ...
  switch (type) {
    ...
    case WS_EVT_DATA: {
        ...
        if (info->final && info->index == 0 && info->len == len && info->opcode == WS_TEXT) {
          ...
          realizarAccion(jsonDoc["accion"],jsonDoc["rpm"]);   
        } 
      }
      break;
...
// ----------
void realizarAccion(int accion, int rpm) {
 
  if (parar) {    

    motores(0,0,0,0);
    // enceder led azul parada
...
void iniciarIna219() {
  ...
  // configure INA219 for averaging at 64 samples, aprox 60 ms
